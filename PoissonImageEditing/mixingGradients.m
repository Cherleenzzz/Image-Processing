function I_result=mixingGradients(I_source,I_background,I1_mask,I2_mask,row_interpolation,col_interpolation,firstpoint)
...
for j=1:col_b
    ...
    for i=1:row_b
        ...
        if(I2_mask(i,j) == 1)             
             ...
             % If |delt(f*)|>|delt(g)|, Vpq=delt(f*),
             % otherwise, Vpq=delt(g)               
             if((abs(I_background(i,j)-I_background(i-1,j)))>abs(I_source(x,y)-I_source(x-1,y)))
                 Vpq1=I_background(i,j)-I_background(i-1,j);
             else
             ...
             % If |delt(f*)|>|delt(g)|, Vpq=delt(f*),
             % otherwise, Vpq=delt(g)               
             if((abs(I_background(i,j)-I_background(i,j-1)))>abs(I_source(x,y)-I_source(x,y-1)))
                 Vpq2=I_background(i,j)-I_background(i,j-1);
             else
             ...
             % otherwise, Vpq=delt(g)               
             if((abs(I_background(i,j)-I_background(i+1,j)))>abs(I_source(x,y)-I_source(x+1,y)))
                 Vpq3=I_background(i,j)-I_background(i+1,j);
             else
             ...
             % otherwise, Vpq=delt(g)               
             if((abs(I_background(i,j)-I_background(i,j+1)))>abs(I_source(x,y)-I_source(x,y+1)))
                 Vpq4=I_background(i,j)-I_background(i,j+1);
             else
